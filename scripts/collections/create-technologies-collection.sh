#!/bin/bash

echo "Creating Technologies collection for Portfolio"

# Set up variables
DATABASE_ID="portfolio"
COLLECTION_ID="technologies"

# Function for creating a collection with proper error handling
create_collection() {
  local db_id=$1
  local coll_id=$2
  local name=$3

  echo "Creating collection: $name ($coll_id)"

  # Try to create collection, ignore if it exists
  appwrite databases create-collection \
    --database-id $db_id \
    --collection-id $coll_id \
    --name "$name" \
    --permissions "read(\"any\")" "write(\"users\")" \
    --json || echo "Collection $coll_id may already exist"

  # Give it time to process
  sleep 2
}

# Process Technologies collection
create_collection $DATABASE_ID "technologies" "Technologies"

# Add attributes for Technologies
appwrite databases create-string-attribute \
  --database-id $DATABASE_ID \
  --collection-id technologies \
  --key name \
  --size 255 \
  --required true \
  --json || echo "Attribute may exist"

appwrite databases create-string-attribute \
  --database-id $DATABASE_ID \
  --collection-id technologies \
  --key category_id \
  --size 255 \
  --required true \
  --json || echo "Attribute may exist"

appwrite databases create-url-attribute \
  --database-id $DATABASE_ID \
  --collection-id technologies \
  --key icon \
  --required false \
  --json || echo "Attribute may exist"

appwrite databases create-url-attribute \
  --database-id $DATABASE_ID \
  --collection-id technologies \
  --key website \
  --required false \
  --json || echo "Attribute may exist"

# Wait longer to ensure attributes are ready
echo "Waiting for technology attributes to be ready..."
sleep 5

# Add index for Technologies
appwrite databases create-index \
  --database-id $DATABASE_ID \
  --collection-id technologies \
  --key by_category \
  --type key \
  --attributes category_id \
  --json || echo "Index may exist or attributes not ready"

echo "Technologies collection created successfully!"